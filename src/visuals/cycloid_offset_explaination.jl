...
step_slider = Slider(f[4, 1], range= -8π/100:8π/2000:8π/100, startvalue=0.15)
...
edge = edge * 1.5
θ = Observable(0.0)
pos = @lift(rotmat($θ)*[0.0, $eccentricity] + [0.0, l2])
rot = @lift(-$θ/4)

cyc = @lift(Cycloid($eccentricity, l2, -1 + 1/5, $rot, [0, 0], $inset))
