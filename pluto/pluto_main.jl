...
# ╔═╡ 0124acd4-6da2-4fd1-8cd7-687c270e2fb9
using CairoMakie

# ╔═╡ 15b0c0bc-a123-4777-bd65-a6877ef763e0
using Symbolics

# ╔═╡ 138fd023-3412-4535-9e37-600512d2e399
using BenchmarkTools

# ╔═╡ f368376c-d6b1-4557-8735-28d44e79d97c
const Float = Float64

# ╔═╡ 4381ab00-6643-467a-b12a-02263ca370da
begin
	const NorD = Union{Num,Float}
	NorD(x::Number) = Float(x)
end

# ╔═╡ dfc3e17c-40d1-4589-9e6f-a0c75453156f
function rotmat(θ::Float)
    return [cos(θ) sin(θ); sin(θ) -cos(θ)]
end

# ╔═╡ 911b5a31-f3c7-4b4d-80b2-e5d37438aa09
begin
	struct Circle
   		pos::Vector{NorD}
    	radius::NorD
	...
	function (circle::Circle)(θ::NorD)
    	rotmat(θ + circle.θ) * [circle.radius; 0] + circle.pos
	end
end

# ╔═╡ f1a1af86-bf9d-4d23-8d45-b734f6bbf180
...
# ╔═╡ a46f52d1-f27c-49f3-aff5-2e16a4dc632d
begin
    mutable struct Cycloid
        l1::Float           # inner element radius
		l2::Float           # outer element radius
		k::Float            # element relative rate
    	θ::NorD             # angle
    	pos::Vector{NorD}   # position
	end

	function (cycloid::Cycloid)(t::NorD)
...
# ╔═╡ 27dabea8-f189-44d9-920e-45ce756c843f
function get_cycloid_points(cycloid::Cycloid, steps::Int)
    step_arr = LinRange(0.0, 2π/(cycloid.k+1), steps)
    points = cycloid.(step_arr)
    return reduce(hcat, points)
end

# ╔═╡ aadb2385-1c61-441e-920b-ae7f2a98fd45
function draw_cycloid(cycloid::Cycloid, steps=1024)
    points = get_cycloid_points(cycloid, steps)
    lines!(points[1, :], points[2, :])
end

# ╔═╡ fcb2e5ce-a3c5-49cc-8b40-0a6c4ec0a3ba
function draw_cycloid(cycloid::Observable{Cycloid}, steps::Int=1024)
    points = @lift(get_cycloid_points($cycloid, steps))
    lines!(@lift($points[1, :]), (@lift($points[2, :])))
end

# ╔═╡ 02b79bff-07ec-4c2f-af01-9dc34560321f
...
# ╔═╡ 8e7ad0a9-faed-4974-9606-855eef65afd6
θ = Observable(0.0)

# ╔═╡ 66f43931-cc8c-4dc5-b691-7ca24f0ea65a
cyc = @lift(Cycloid(0.25, 1.0, -1.1, $θ, [0.0; 0.0]))

# ╔═╡ c902ea8c-e4ae-450b-b9ec-fcf42c0c2eaa
θ_it = LinRange(0, 2π, 512)

# ╔═╡ e2b3b7b4-52b0-4ec0-88c7-7a8c0886c3b3
set_theme!(theme_black())

# ╔═╡ a5f82aa1-b1c6-4a65-909d-c515244d2769
begin
	cyc[].θ = 0
...
# ╔═╡ 5ea38319-480f-4078-8057-3590d11e3cba
# ╠═╡ disabled = true
#=╠═╡
CairoMakie.Makie.Record(f, θ_it, framerate = 60) do θ_i
    θ[] = θ_i
end
  ╠═╡ =#

# ╔═╡ abb701f0-f69c-4c8d-bfe5-706033c5d41c
f

# ╔═╡ 3db9b4c0-eb2a-4b7c-8c99-e3d3e3ad3574
@benchmark get_cycloid_points(cyc[], 1024)
...
git-tree-sha1 = "6762905849b462dd6241faf3542696a002de021f"
...
git-tree-sha1 = "eba536e02079a3e2368ea3638ece08e410c5a964"
